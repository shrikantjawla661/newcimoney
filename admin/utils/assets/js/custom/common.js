...
if (exportCsvBtn) {
  exportCsvBtn.addEventListener("click", async function (e) {
    $("#loader").show();
    await $.ajax({
      url: `/admin/${e.target.dataset.table}/export-csv`,
      type: "post",
      data: {
...
function getSelectFields(table) {
  return selectFieldsArray.map((elem) => {
    return $.ajax({
      url: `/admin/factory/distinctValues?table=${table}&column=${elem}`,
      type: "GET",
      contentType: "application/json",
      dataType: "json",
      success: function (result) {
...
async function getTeleCallers() {
  return $.ajax({
    url: `/admin/users/get-telecallers-ajax`,
    type: "GET",
    contentType: "application/json",
    dataType: "json",
    success: function (result) {
...
document.getElementById("assign-submit")?.addEventListener("click", () => {
  ...
  $.ajax({
    url: "/admin/factory/add-remove-telecallers-permissions",
    method: "POST",
    contentType: "application/json",
    dataType: "json",
    data: JSON.stringify({
...
document.getElementById("reassign-submit")?.addEventListener("click", () => {
  ...
  if ($('#reassignPermissionSelectFrom').select2('data').length == 1 && $('#reassignPermissionSelectTo').select2('data').length == 1 && $('#reassignPermissionSelectFrom').select2('data')[0].id != $('#reassignPermissionSelectTo').select2('data')[0].id) {
    ...
    $("#showError").html("");
    $.ajax({
      url: "/admin/factory/reassign-telecaller-permission",
      method: "POST",
      contentType: "application/json",
      dataType: "json",
      data: JSON.stringify({
...
const addSelectEventListeners = () => {
  ...
  $(".row-telecallers").on("select2:select", function (e) {
    ...
    $.ajax({
      url: "/admin/factory/add-remove-single-telecaller-permission",
      method: "POST",
      contentType: "application/json",
      dataType: "json",
